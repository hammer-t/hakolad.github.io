...
var Point = function(){
  ...
  this._step = function(aCollection){
    ...
    var modifiedVelocity = this._velocity*0.3 * ((Math.random() * this._randomization+1)/10);
  ...
  this.draw = function(context){
    ...
    for(i=0; i<this._neighboors.length; i++) {
        ...
        context.lineWidth = 0.5 + 5 / this._neighboors[i].distance;
    }
    context.closePath();
...
function init() {
  can = document.getElementById("realisation-canvas");
...
if( document.getElementById("realisation-canvas") != null ) {
  init();
}